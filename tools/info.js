function infoTool(toolbar) {
    ...
    canvas = ElementHelper.create("canvas", {"width": toolbar.retina ? 320 : 160, "height": toolbar.retina ? 252 : 126});
    ...
    function updateInfo(text, row) {
        ...
        ctx.fillText(text, categoryEnd + (toolbar.retina ? 4 : 2), textHeight * row);
    }
    ...
    function update(block) {
        ...
        if (block.isBlocky) {
            updateInfo(block.upperBlockColor, 7);
            updateInfo(block.lowerBlockColor, 8);
        } else {
            updateInfo("N/A", 7);
            updateInfo("N/A", 8);
        }
